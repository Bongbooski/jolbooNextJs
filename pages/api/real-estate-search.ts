...
export default async function handler(
  ...
  try {
    ...
    try {
        debugger;
      // RAG 시스템을 통한 실제 데이터 생성
      const realEstateData = await ragSystem.generateRealEstateData(districtStr, areaNum, maxPriceNum);
      
      res.status(200).json({
        success: true,
        data: realEstateData,
        source: 'rag-system',
        message: 'Azure AI Search RAG 시스템을 통해 생성된 데이터입니다.'
      });
    } catch (error) {
      console.error('RAG 시스템 오류:', error);
      res.status(500).json({ 
        success: false,
        message: 'RAG 시스템에서 데이터를 생성하는 중 오류가 발생했습니다.',
